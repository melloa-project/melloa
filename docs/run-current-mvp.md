...
8. Open **Memory**, paste the printed seed assertion ID, and inspect its value and append-only state history. Corrections and content deletion require the recent in-memory mutation proof established at login. On the default process-local path, select **Delete content**, confirm, and verify the assertion value is gone while metadata, state history, tombstone ID, rebuild work ID, and the reported backup-expiry state remain visible. In the default fixture that state is **Not Configured**. In PostgreSQL mode, assertion content deletion is not installed yet and must fail closed; use correction/dispute/retraction for the restart-durability drill.
...
- PostgreSQL mode durably backs canonical conversation, retrieval/model provenance, memory correction history, reply/delivery work, Telegram pairing and revocation authority, normalized updates/receipts, monotonic poll offsets, and pre-submission reply reconstruction. Owner sessions, provider health, Telegram challenge-send observation, attachment quarantine bytes, retention inventory, event/audit emission, and backup remain process-local, synthetic, unassembled, unstored, or unconfigured as reported. Owner assertion content deletion is currently implemented for the process-local memory store only; the durable PostgreSQL path rejects it rather than claiming content erasure without the installed deletion procedure.
...
- Capture is disabled, media content is never served, backup is unconfigured, and retention inventory is synthetic.
